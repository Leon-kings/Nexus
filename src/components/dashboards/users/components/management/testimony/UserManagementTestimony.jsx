...
export const UserTestimonyDashboard = () => {
  // States for data
  ...
  // Initialize user data and axios
  useEffect(() => {
    ...
    if (userData) {
      try {
        ...
        setFormData(prev => ({
          ...
          position: parsedUser.position || parsedUser.status || ''
        }));
      } catch (error) {
  ...
  // Filter testimonials based on search and view mode
  useEffect(() => {
    let filtered = testimonials;

    // Filter by view mode
    if (viewMode === 'my' && currentUser) {
      filtered = testimonials.filter(testimonial => 
        testimonial.userId === currentUser._id || 
        testimonial.name?.toLowerCase() === currentUser.name?.toLowerCase() ||
        testimonial.name?.toLowerCase() === currentUser.username?.toLowerCase()
      );
    }
  ...
  }, [searchTerm, testimonials, viewMode, currentUser]);
  ...
  const resetForm = () => {
    ...
    setFormData({
      ...
      position: currentUser?.position || currentUser?.status || '',
      quote: '',
      image: null
    });
    setPreviewImage('');
  ...
  const openEditModal = (testimonial) => {
    ...
    if (!canUserEditTestimonial(testimonial)) {
      showError('You can only edit your own testimonials');
      return;
    }
  ...
  // Check if user can edit/delete testimonial
  const canUserEditTestimonial = (testimonial) => {
  ...
  const openDeleteModal = (testimonial) => {
    ...
    if (!canUserEditTestimonial(testimonial)) {
      showError('You can only delete your own testimonials');
      return;
    }
  ...
  // Get user's testimonials count
  const getUserTestimonialsCount = () => {
    if (!currentUser) return 0;
    return testimonials.filter(testimonial => 
      canUserEditTestimonial(testimonial)
    ).length;
  };
            ...
            <div>
              ...
              {currentUser && (
                <p className="text-sm text-gray-600 mt-1">
                  Adding as: <span className="font-medium">{currentUser.name || currentUser.username}</span>
                </p>
              )}
            </div>
            <button
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                {isEdit ? 'Select new image to replace current one' : 'Upload your profile image (max 5MB)'}
              </p>
            </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                {currentUser ? `Welcome, ${currentUser.name || currentUser.username}!` : 'Manage and organize customer testimonials'}
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex bg-gray-100 rounded-lg p-1">
                ...
                >
                  ...
                  My Testimonials ({getUserTestimonialsCount()})
                </button>
              </div>
              
              <button
                onClick={openCreateModal}
                className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-300 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                type="button"
              >
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 mb-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder={`Search ${viewMode === 'my' ? 'my' : ''} testimonials by name, position, or quote...`}
                value={searchTerm}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-gray-900">
              {viewMode === 'my' ? 'My Testimonials' : 'All Testimonials'} ({filteredTestimonials.length})
            </h2>
            {searchTerm && (
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {searchTerm ? 'No testimonials found' : `No ${viewMode === 'my' ? 'my' : ''} testimonials yet`}
              </h3>
              <p className="text-gray-600 mb-6">
                {searchTerm 
                  ? 'Try adjusting your search terms' 
                  : viewMode === 'my' 
                    ? 'Share your experience by adding your first testimonial'
              ...
              {!searchTerm && (
                ...
                >
                  <Add className="w-5 h-5" />
                  {viewMode === 'my' ? 'Add My First Testimonial' : 'Add First Testimonial'}
                </button>
              )}
            </div>
          ) : (
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {filteredTestimonials.map((testimonial) => {
                const canEdit = canUserEditTestimonial(testimonial);
                return (
                  <div
                    key={testimonial._id}
                    className={`bg-gradient-to-br from-white to-gray-50 rounded-xl border ${
                      canEdit ? 'border-blue-200' : 'border-gray-200'
                    } p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 relative`}
                  >
                    {/* Ownership Badge */}
                    {canEdit && (
                    ...
                    <div className="flex gap-2 mt-4 pt-4 border-t border-gray-200">
                      <button
                        ...
                        disabled={!canEdit}
                        className={`flex-1 flex items-center justify-center gap-1 px-3 py-2 rounded-lg transition-all duration-300 text-sm font-medium shadow-sm hover:shadow-md ${
                          canEdit
